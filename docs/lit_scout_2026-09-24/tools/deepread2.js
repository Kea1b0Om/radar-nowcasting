...
const items = args.map(id => (typeof id === 'string') ? { arxiv_id: id, title: '(见 ctx 文件)', why: `发现阶段的笔记在 ${SP}/ctx/${id}.json（先 cat 它；注意其中的数字来自搜索摘要或其他代理，不可直接当证据，必须回到 PDF 核对）`, tasks: [] } : id)
